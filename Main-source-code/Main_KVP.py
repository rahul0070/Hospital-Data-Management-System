...
def check():
    ...
    cur = conn.execute("select file_no from Record_KVP")
    for i in cur:
        if (i[0] == rec):
            flag = 1
    if (flag == 1):
...
def submit():
    ...
    d = e5.get()
    diagnosis = e6.get()
    conn.execute("insert into Record_KVP (name, age, file_no, doe, sex, diag, phone, occupation) values (?,?,?,?,?,?,?,?);",
...
l1 = Label(fr, text="PATIENT RECORD KOVILPATTI")
