...
def fileg(reg):
    file_name = 'Case_files_TVL/' + str(reg) + ".txt"
    lines = []
...
def update():
    reg = ex.get()
    conn = sqlite3.connect('Record_database.db')
    conn.execute('update Record_TV SET name=?, age=?, doe=?, sex=?, diag=?, phone=?, occupation=? where file_no=?', (e1.get(), e2.get(), e5.get(), e4.get(), e6.get(), e7.get(), e8.get(), reg))
    conn.commit()
    conn.close()
    file_name = 'Case_files_TVL/' + reg + ".txt"
    os.remove(file_name)
